<x-main>
    ...
    <x-slot name="bodyContent">
        <div class="card card-custom">
            <div class="card-header">
                <div class="card-title">
                    ...
                    <h3 class="card-label">Listing des Pet owners</h3>
                </div>
        ...
        {{--        add petowner modal--}}
        <x-modal modal-title="Ajouter un pet owner" method="POST" route="petowner.store" form-id="addForm"
                 modal-name="addModal">
        ...
        {{--        update petowner modal--}}
        <x-modal modal-title="Modifier un pet owner" method="POST" route="petowners.update" form-id="updateForm"
    ...
    <x-slot name="jsSlot">
        ...
        <script>
            ...
            $(document).on('shown.bs.modal', '#roleModalUPetOwner', function (evt) {
                select.trigger("change");
                $('.select2-ajax').trigger("change");
            });
        </script>
        <x-js-data-table route='{{route("petowners.index")}}' tableId="kt_datatable">

            <x-slot name="jsonColumns">
                {!! file_get_contents(public_path("js/pages/petowner/petowners_datatable.js")) !!}
            </x-slot>
        </x-js-data-table>
        <script>
            var id;
            ...
            $('body').on('click', '.edit', function () {
                id = $(this).data("id");
                $("#updateForm").attr("action", "{{route("petowners.update",["petowner"=>""])}}" + "/" + id);
                ...
                swal.fire({
                    ...
                    if (willDelete) {
                        $.ajax({
                            ...
                            url: "{{route('petowners.desactivateAccount',["petowner" => ''])}}" + "/" + id,
